...
//         vm.prank(_user);
//         _terminals[0].pay{value: 1 ether}(_projectId, 1 ether, address(0), _user, 0, true, "", metadata);
...
//             JBTiered721SetTierDelegatesData[] memory tiered721SetDelegatesData =
//                 new JBTiered721SetTierDelegatesData[](1);
//             tiered721SetDelegatesData[0] =
//                 JBTiered721SetTierDelegatesData({delegatee: _users[i], tierId: uint256(i + 1)});
//             vm.prank(_users[i]);
...
//         vm.warp(block.timestamp + (_governor.votingPeriod() * 12) + 1);
...
//     ) public {
//         vm.assume(
//             _durationUntilProjectLaunch > 2 && _mintDuration > 1 && _inBetweenMintAndFifa > 1 && _fifaDuration > 1
//         );
...
//         internal
//         returns (uint256 projectId, DefifaDelegate nft, DefifaGovernor governor)
//     {
...
//             uint256[] memory redemptionId = new uint256[](1);
//             redemptionId[0] =
//                 _generateTokenId(_tierId, _tier.initialQuantity - _tier.remainingQuantity + 1 + _numberBurned);
//             redemptionMetadata = abi.encode(bytes32(0), type(IDefifaDelegate).interfaceId, redemptionId);
//         }
//         vm.prank(_refundUser);
//         JBETHPaymentTerminal(address(_terminals[0])).redeemTokensOf({
