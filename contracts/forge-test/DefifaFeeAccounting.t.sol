...
contract DefifaFeeAccountingTest is JBTest, TestBaseWorkflow {
    ...
    function testFeeAccounting_defaultSplits() external {
        ...
        uint256 potBefore = jbMultiTerminal().STORE().balanceOf(
        ...
        uint256 potAfter = jbMultiTerminal().STORE().balanceOf(
            address(jbMultiTerminal()), projectId, JBConstants.NATIVE_TOKEN
        );

        assertEq(potAfter, expectedSurplus, "surplus after fees should be pot - feeAmount");
        assertEq(deployer.fulfilledCommitmentsOf(projectId), expectedFee, "fulfilledCommitmentsOf should equal fee amount");
    ...
    function _getDefifaLaunchDataWithSplits(uint8 nTiers, JBSplit[] memory splits) internal returns (DefifaLaunchProjectData memory) {
    ...
    function _mintAllTiers(DefifaHook _nft, DefifaGovernor _governor, uint256 projectId, uint8 nTiers)
        internal
        returns (address[] memory users)
    {
        ...
        for (uint256 i = 0; i < nTiers; i++) {
            ...
            vm.prank(users[i]);
            jbMultiTerminal().pay{value: 1 ether}(projectId, JBConstants.NATIVE_TOKEN, 1 ether, users[i], 0, "", metadata);
