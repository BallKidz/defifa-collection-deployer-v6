...
/// @title SVG2
/// @notice Minimal SVG token URI resolver derived from the original HTML resolver but without on-chain font imports.
contract SVG2 is Ownable, IDefifaTokenUriResolver, IJB721TokenUriResolver {
    ...
    /// @inheritdoc IJB721TokenUriResolver
    function tokenUriOf(address _nft, uint256 _tokenId) external view override returns (string memory) {
        return _buildTokenUri(_nft, _tokenId);
    }

    function _buildTokenUri(address _nft, uint256 _tokenId) private view returns (string memory) {
        IDefifaDelegate _delegate = IDefifaDelegate(_nft);
        uint256 _gameId = _getGameId(_delegate, _nft);
        ...
        return _buildSvgTokenUri(_delegate, _store, _tier, _tokenId, _gameId);
    }

    function _getGameId(IDefifaDelegate _delegate, address _nft) private view returns (uint256) {
        try _delegate.PROJECT_ID() returns (uint256 gameId_) {
            if (gameId_ != 0) return gameId_;
        } catch {}

        if (_gameIdOverrideSet[_nft]) {
            return _gameIdOverride[_nft];
        }

        return 0;
    }

    function _buildSvgTokenUri(
        ...
        (bool _phaseFetched, DefifaGamePhase _gamePhase) = _safeGamePhase(_phaseReporter, _gameId);
        (bool _potFetched, uint256 _gamePot, address _gamePotToken, uint256 _gamePotDecimals) = _safeGamePot(_potReporter, _gameId);
        ...
        _gamePot = _gamePot + _amountRedeemed;
        ...
        string memory _gamePhaseText = _getGamePhaseText(_phaseFetched, _gamePhase);
        string memory _rarityText = _getRarityText(_tier, _phaseFetched, _gamePhase);
        string memory _valueText =
            _getValueText(_delegate, _tier, _tokenId, _phaseFetched, _gamePhase, _gamePot, _gamePotToken, _gamePotDecimals);
        ...
        parts[2] = Base64.encode(
            abi.encodePacked(
                ...
                _potText,
                " | CARDS: ",
                _getTotalSupply(_store, address(_delegate)),
                "</text>",
    ...
    function _getGamePhaseText(bool _phaseFetched, DefifaGamePhase _gamePhase) private pure returns (string memory) {
        if (!_phaseFetched) return "Phase data unavailable.";
        if (_gamePhase == DefifaGamePhase.NO_CONTEST) return "No contest. Refunds open.";
        if (_gamePhase == DefifaGamePhase.NO_CONTEST_INEVITABLE) return "No contest inevitable. Refunds open.";
        if (_gamePhase == DefifaGamePhase.COUNTDOWN) return "Minting starts soon.";
        if (_gamePhase == DefifaGamePhase.MINT) return "Minting and refunds are open.";
        if (_gamePhase == DefifaGamePhase.REFUND) return "Minting is over. Refunds are ending.";
        if (_gamePhase == DefifaGamePhase.SCORING) return "Awaiting scorecard approval.";
        if (_gamePhase == DefifaGamePhase.COMPLETE) return "Scorecard locked in. Burn to claim reward.";
        return "Phase data unavailable.";
    }

    function _getRarityText(JB721Tier memory _tier, bool _phaseFetched, DefifaGamePhase _gamePhase)
        private
        pure
        returns (string memory)
    {
        ...
        if (_phaseFetched && _gamePhase == DefifaGamePhase.MINT) {
            return string(abi.encodePacked(_totalMinted.toString(), _totalMinted == 1 ? " card so far" : " cards so far"));
        }
        return string(
            abi.encodePacked(_totalMinted.toString(), _totalMinted == 1 ? " card in existence" : " cards in existence")
        );
    }

    function _getValueText(
        IDefifaDelegate _delegate,
        JB721Tier memory _tier,
        uint256 _tokenId,
        bool _phaseFetched,
        ...
        uint256 _gamePot,
        address _gamePotToken,
        uint256 _gamePotDecimals
    ) private view returns (string memory) {
        if (_phaseFetched && (_gamePhase == DefifaGamePhase.SCORING || _gamePhase == DefifaGamePhase.COMPLETE)) {
            ...
            if (_totalRedemptionWeight != 0) {
                uint256 _potPortion = PRBMath.mulDiv(_gamePot, _redemptionWeight, _totalRedemptionWeight);
                return !_redemptionSet
                    ? "Awaiting scorecard..."
                    : _formatBalance(_potPortion, _gamePotToken, _gamePotDecimals, _IMG_DECIMAL_FIDELITY);
            }
        }
        return _formatBalance(_tier.price, _gamePotToken, _gamePotDecimals, _IMG_DECIMAL_FIDELITY);
    }
    ...
    function _getTotalSupply(IJBTiered721DelegateStore _store, address _delegate) private view returns (string memory) {
        try _store.totalSupplyOf(_delegate) returns (uint256 supply) {
