...
/// @title L50_FeeTokenDonation
/// @notice Regression test: donated tokens should not inflate fee token claims.
contract L50_FeeTokenDonation is JBTest, TestBaseWorkflow {
    ...
    /// @notice Test that donated tokens do not inflate claims.
    /// @dev After cash-out weights are set, directly transferring fee tokens to the hook
    ///      should NOT increase the claimable amount for any user.
    function test_donatedTokensDontInflateClaims() public {
        uint8 nTiers = 4;
        ...
        // Build scorecards - equal weights
        ...
        // Record claimable amounts BEFORE donation
        uint256[] memory tokenIds0 = new uint256[](1);
        tokenIds0[0] = _generateTokenId(1, 1);
        (uint256 defifaClaimBefore, uint256 baseProtocolClaimBefore) = _nft.tokensClaimableFor(tokenIds0);

        // Donate extra fee tokens directly to the hook
        (uint256 defifaAlloc, uint256 baseAlloc) = _nft.tokenAllocations();
        // Mint some tokens and donate them
        deal(_defifaProjectTokenAccount, address(this), 1000 ether);
        deal(_protocolFeeProjectTokenAccount, address(this), 1000 ether);
        IERC20(_defifaProjectTokenAccount).transfer(address(_nft), 1000 ether);
        IERC20(_protocolFeeProjectTokenAccount).transfer(address(_nft), 1000 ether);

        // Record claimable amounts AFTER donation
        (uint256 defifaClaimAfter, uint256 baseProtocolClaimAfter) = _nft.tokensClaimableFor(tokenIds0);

        // The claimable amounts should NOT have increased due to the donation
        assertEq(defifaClaimBefore, defifaClaimAfter, "Defifa claim should not increase from donation");
        assertEq(
            baseProtocolClaimBefore, baseProtocolClaimAfter, "Base protocol claim should not increase from donation"
        );

        // tokenAllocations() should also report tracked (non-inflated) amounts
        (uint256 defifaAllocAfter, uint256 baseAllocAfter) = _nft.tokenAllocations();
        assertEq(defifaAlloc, defifaAllocAfter, "tokenAllocations defifa should not increase from donation");
        assertEq(baseAlloc, baseAllocAfter, "tokenAllocations base should not increase from donation");
    }
